	...
	public static function nam_tech_settings_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
                ...
                //select option
                case 'posts':
                    $output .= '<div class="select-theme-wrapper ' . $mini . $position. '">';
                    $output .= '<select class="select-theme-option" name="'.$value['id'].'" id="'. $value['id'] .'">';
                    ...
                    if($value['postType']) {
                        $args['post_type'] = $value['postType'];
                    }
                    $the_query = new WP_Query($args);
                    if($the_query->have_posts()){
                        while($the_query->have_posts()) {
                            $the_query->the_post();
                            $output .= '<option value="'.get_the_ID().'" ' . selected($smof_data[$value['id']], get_the_ID(),false) . ' >'.get_the_title().'</option>';
                        }
                    }
                    wp_reset_postdata();
                    $output .= '</select></div>';
